...
export function ActivityLog() {
  ...
  return (
    <div
      className={`bg-white border-l border-slate-200 transition-all duration-300 flex ${
        isOpen ? 'w-80' : 'w-12'
      }`}
          ...
          <ScrollArea className="flex-1">
            <div className="p-4 space-y-3">
              {activityLog.length === 0 ? (
                <div className="text-sm text-slate-400 text-center py-8" data-testid="empty-activity-log">
                  No activity yet
                </div>
              ) : (
